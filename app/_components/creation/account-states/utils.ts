...
type AccountStatesState = AccountStatesResponse & { mediaType: MediaType } | null;
...
const upsertState = (
    ...
    value: Partial<NonNullable<AccountStatesState>> | keyof NonNullable<AccountStatesState>
): AccountStatesState => {
    if (!state) return null;
    return Object.assign(state, typeof value === 'string' ? { [value]: !state[value] } : value);
}

const reducer = (state: AccountStatesState, action: AccountStatesAction): AccountStatesState => {
    switch (action.type) {
...
export const StatesContext = createContext<ReturnType<typeof useStatesReducer>>([null, () => void 0]);